...
public enum CardColor {
	...
	for (CardColor cardColor : CardColor.values()) {
	    if (cardColor.getDiv() == colorId) {
		return cardColor;
	    }
	}

	logger.warn(Message.INVALID_CARD_COLOR_DIV.getMessageString(new Object[] { colorId }));
	return CardColor.UNKNOWN;
    }

    public static int getColorIdFromCardId(int cardId) {
	return cardId % 4;
    }

    public static CardColor getColorFromCardId(int cardId) {
	return decodeByColorId(getColorIdFromCardId(cardId));
    }
}
