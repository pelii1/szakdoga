...
public enum CardValue {
	...
	for (CardValue cardValue : CardValue.values()) {
	    if (cardValue.getModulo() == cardValueId) {
		return cardValue;
	    }
	}

	logger.warn(Message.INVALID_CARD_VALUE_CODE.getMessageString(new Object[] { cardValueId }));
	return CardValue.UNKNOWN;
    }

    public static int getCardValueId(int card) {
	return card / 13;
    }

    public static CardValue getValueFromCardId(int card) {
	return decodeByCardValueId(getCardValueId(card));
    }
}
